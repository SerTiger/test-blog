...
<div class="form-group">
    <label for="{!! $locale . '[h1_tag]' !!}" class="col-sm-2 control-label">
        {!! Form::label($locale.'[h1_tag]', __('admin_labels.h1_tag'), array('class' => "control-label")) !!}
    </label>

    <div class="col-xs-6">
        {!! Form::text($locale.'[h1_tag]', isset($model->translate($locale)->h1_tag) ? $model->translate($locale)->h1_tag : '', array('placeholder' => __('admin_labels.h1_tag'), 'class' => 'form-control input-sm name_'.$locale)) !!}
        {!! $errors->first($locale.'.h1_tag', '<span class="error">:message</span>') !!}
    </div>
</div>
